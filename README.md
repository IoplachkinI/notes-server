...
В корневой директории находится файл `docker-compose.yml`, через который все и запускается
Есть несколько compose-профилей:
 - `debug` - запускает только `notes-server`. Также поддерживает hot-reload (для разработки)
 - `release-nginx` - запускает 3 инстанса `notes-server`, Nginx в качестве балансировщика
 - `release-custom-balancer` - запускает 3 инстанса `notes-server` с кастомным балансировщиком
 - `release-multi-tier` - запускает 6 инстансев `notes-server`, по два балансировщика на 3 инстанса и сверху этих двух еще один балансировщик
