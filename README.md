...
Есть несколько compose-профилей:
 ...
 - `docker-compose.multi-tier.yml` - запускает 6 инстансев `notes-server`, по два балансировщика на 3 инстанса и сверху этих двух еще один балансировщик. Имеет 2 профиля: `http` и `https`, порты для обоих одинаковые

 Запускается командой `docker compose -f <file-name> --profile <your-profile (opional)> up --build`
...
В `/note-server/certs` можно найти все сгенерированные сертификаты, а именно, корневой сертификат и подписанный им сертификат сервера. Это позволяет поддерживать https на нашем сервере (через nginx или собственный балансировщик, без прокси оно работать не будет)
